...
__global__ void radixSortLocalKernel(data_t *keys, data_t *values, uint_t bitOffset)
{
    ...
    const uint_t elemsPerThreadBlock = threadsSortLocal * ELEMS_PER_THREAD_LOCAL_KV;
    ...
    // Every thread processes ELEMS_PER_THREAD_LOCAL_KV elements
    for (uint_t shift = bitOffset; shift < bitOffset + bitCountRadix; shift++)
    {
        ...
        index = ELEMS_PER_THREAD_LOCAL_KV * threadIdx.x;
...
#if (ELEMS_PER_THREAD_LOCAL_KV >= 1)
        index = pred0 ? trueBefore + falseTotal : (ELEMS_PER_THREAD_LOCAL_KV * threadIdx.x) - trueBefore;
        keysTile[index] = key0;
        valuesTile[index] = val0;
#endif
#if (ELEMS_PER_THREAD_LOCAL_KV >= 2)
        trueBefore += pred0;
        index = pred1 ? trueBefore + falseTotal : (ELEMS_PER_THREAD_LOCAL_KV * threadIdx.x + 1) - trueBefore;
        keysTile[index] = key1;
        valuesTile[index] = val1;
#endif
#if (ELEMS_PER_THREAD_LOCAL_KV >= 3)
        trueBefore += pred1;
        index = pred2 ? trueBefore + falseTotal : (ELEMS_PER_THREAD_LOCAL_KV * threadIdx.x + 2) - trueBefore;
        keysTile[index] = key2;
        valuesTile[index] = val2;
#endif
#if (ELEMS_PER_THREAD_LOCAL_KV >= 4)
        trueBefore += pred2;
        index = pred3 ? trueBefore + falseTotal : (ELEMS_PER_THREAD_LOCAL_KV * threadIdx.x + 3) - trueBefore;
        keysTile[index] = key3;
        valuesTile[index] = val3;
#endif
#if (ELEMS_PER_THREAD_LOCAL_KV >= 5)
        trueBefore += pred3;
        index = pred4 ? trueBefore + falseTotal : (ELEMS_PER_THREAD_LOCAL_KV * threadIdx.x + 4) - trueBefore;
        keysTile[index] = key4;
        valuesTile[index] = val4;
#endif
#if (ELEMS_PER_THREAD_LOCAL_KV >= 6)
        trueBefore += pred4;
        index = pred5 ? trueBefore + falseTotal : (ELEMS_PER_THREAD_LOCAL_KV * threadIdx.x + 5) - trueBefore;
        keysTile[index] = key5;
        valuesTile[index] = val5;
#endif
#if (ELEMS_PER_THREAD_LOCAL_KV >= 7)
        trueBefore += pred5;
        index = pred6 ? trueBefore + falseTotal : (ELEMS_PER_THREAD_LOCAL_KV * threadIdx.x + 6) - trueBefore;
        keysTile[index] = key6;
        valuesTile[index] = val6;
#endif
#if (ELEMS_PER_THREAD_LOCAL_KV >= 8)
        trueBefore += pred6;
        index = pred7 ? trueBefore + falseTotal : (ELEMS_PER_THREAD_LOCAL_KV * threadIdx.x + 7) - trueBefore;
        keysTile[index] = key7;
