...
int main(int argc, char **argv)
{
    ...
    allocDeviceMemory(&d_dataTable, &d_dataBuffer, &d_ranksEven, &d_ranksOdd, tableLen);
    ...
    for (uint_t i = 0; i < testRepetitions; i++)
    {
        ...
        timers[SORT_PARALLEL][i] = sortParallel(
            h_outputParallel, d_dataTable, d_dataBuffer, d_ranksEven, d_ranksOdd, tableLen, sortOrder
        );
    ...
    freeDeviceMemory(d_dataTable, d_dataBuffer, d_ranksEven, d_ranksOdd);
