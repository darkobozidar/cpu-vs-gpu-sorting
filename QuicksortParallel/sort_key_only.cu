...
uint_t QuicksortParallel::runMinMaxReductionKernel(data_t *d_keys, data_t *d_keysBuffer, uint_t arrayLength)
{
    ...
    uint_t sharedMemSize = 2 * THREADS_PER_REDUCTION_KO * sizeof(data_t);
    dim3 dimGrid((arrayLength - 1) / (THREADS_PER_REDUCTION_KO * ELEMENTS_PER_THREAD_REDUCTION_KO) + 1, 1, 1);
    dim3 dimBlock(THREADS_PER_REDUCTION_KO, 1, 1);

    minMaxReductionKernel<<<dimGrid, dimBlock, sharedMemSize>>>(d_keys, d_keysBuffer, arrayLength);
...
)
{
    ...
    if (arrayLength > THRESHOLD_REDUCTION_KO)
    {
...
)
{
    ...
    minMaxReduction(
        h_keys, d_keys, (data_t*)d_keysBuffer, h_minMaxValues, arrayLength, minVal, maxVal
    );
    ...
    while (generateSequences)
    {
        ...
        runQuickSortGlobalKernel(
            d_keys, d_keysBuffer, h_globalSeqDev, d_globalSeqDev, h_globalSeqIndexes,
    ...
    runQuickSortLocalKernel<sortOrder>(
        d_keys, d_keysBuffer, h_localSeq, d_localSeq, numSeqLocal
    );
}
