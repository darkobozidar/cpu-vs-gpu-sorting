...
__global__ void minMaxReductionKernel(data_t *input, data_t *output, uint_t tableLen)
{
    ...
    uint_t elemsPerBlock = THREADS_PER_REDUCTION_KO * ELEMENTS_PER_THREAD_REDUCTION_KO;
    ...
    for (uint_t tx = threadIdx.x; tx < dataBlockLength; tx += THREADS_PER_REDUCTION_KO)
    {
    ...
    if (threadIdx.x < THREADS_PER_REDUCTION_KO / 2)
    {
        warpMinReduce<THREADS_PER_REDUCTION_KO>(minValues);
    }
    else
    {
        warpMaxReduce<THREADS_PER_REDUCTION_KO>(maxValues);
    }
    __syncthreads();
    ...
    if ((threadIdx.x >> WARP_SIZE_LOG) == 0)
    {
        ...
        if (index < THREADS_PER_REDUCTION_KO && THREADS_PER_REDUCTION_KO > WARP_SIZE)
        {
            minValues[threadIdx.x] = minValues[index];
            maxValues[threadIdx.x] = maxValues[index];

            if (index < THREADS_PER_REDUCTION_KO / 2)
            {
                warpMinReduce<(THREADS_PER_REDUCTION_KO >> (WARP_SIZE_LOG + 1))>(minValues);
            }
            else
            {
                warpMaxReduce<(THREADS_PER_REDUCTION_KO >> (WARP_SIZE_LOG + 1))>(maxValues);
            }
        }
