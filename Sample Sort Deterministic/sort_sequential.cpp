...
void mergeSort(data_t *dataTable, data_t *dataBuffer, data_t *dataResult, uint_t tableLen)
{
    ...
    for (uint_t sortedBlockSize = 2; sortedBlockSize <= tableLenPower2; sortedBlockSize *= 2)
    {
        ...
        // If it is last phase, data is coppied to result array
...
)
{
    ...
    // For all elements in data table searches, which bucket they belong to
    ...
    uint_t prevElem = bucketSizes[0];
    bucketSizes[0] = 0;

    for (uint_t i = 1; i <= NUM_SPLITTERS_SEQUENTIAL; i++)
    {
        uint_t currElem = bucketSizes[i];
        bucketOffsets[i] = bucketSizes[i - 1] + prevElem;
        prevElem = currElem;
    }
